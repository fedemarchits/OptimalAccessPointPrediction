"""Ablation arm: dual-branch DINOv2 ViT-B/14 (image + OSM tabular)."""
import sys
...
cfg = DualBranchConfig(
    ...
    wandb_name="dual_dinov2_vitb14",
    patience=7,
)
cfg.warmup_epochs = 3   # DualBranchConfig has no warmup field; set post-init
                        # Trainer reads via getattr(config, "warmup_epochs", 0)
