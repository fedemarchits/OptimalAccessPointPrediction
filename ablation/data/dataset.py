...
class CachedDataset(Dataset):
    ...
    def __init__(
        ...
        # Load everything into RAM (fast for large-RAM machines, ~15GB total)
        print(f"  Loading cache into RAM…", flush=True)
        self._rgb     = np.array(np.lib.format.open_memmap(cache / "rgb.dat",     mode="r"))
        self._dem     = np.array(np.lib.format.open_memmap(cache / "dem.dat",     mode="r"))
        self._landuse = np.array(np.lib.format.open_memmap(cache / "landuse.dat", mode="r"))
        self._targets = np.load(cache / "targets.npy")
        self._tabular = np.load(cache / "tabular.npy")
        print(f"  Cache loaded.", flush=True)
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        return {
            ...
            "target_raw": torch.tensor([float(np.expm1(target))], dtype=torch.float32),
            "lat":        0.0,
            "lon":        0.0,
            "metadata": {
                "city":    meta["city"],
